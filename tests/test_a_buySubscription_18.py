...
# one service provider has two Agents with differetn tariffs. Buy ticket for ether and call agent buySubscription method. With Agent. Ticket is with expiring time 
# and count lefts
def test_buy_subscription(accounts, dai, weth, sub_reg, minter2, agent, AgentForService):
	...
	minter2.registerServiceTariff(tariff1,{'from':accounts[0]})
	#register agent - separate agent
	minter2.authorizeAgentForService(agent_1.address, [1],{"from": accounts[0]})
	...
	assert sub_reg.getTicketPrice(minter2.address, 1,1)[1] == pay_amount

	#pay for ether 
	with reverts("Only one valid ticket at time"):
		agent_1.buySubscription(minter2.address, 1, 1, accounts[1], accounts[1], {"from": accounts[1], "value": pay_amount})
	...
	with reverts("Valid ticket not found"):
		minter2.mint(3, {"from": accounts[1]})
