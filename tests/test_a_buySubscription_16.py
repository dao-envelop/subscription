...
# two service providers have the same Agent. Buy ticket for ether and call agent buySubscription method. With Agent. Ticket is with expiring time 
# and count lefts
def test_buy_subscription(accounts, dai, weth, sub_reg, minter2, agent, MinterServiceWithAgent):
	...
	#####deploy second service provider
	...
	#register agent - separate agent
	minter2_1.authorizeAgentForService(agent.address, [0],{"from": accounts[0]})
	...
	#pay for ether 
	agent.buySubscription(minter2_1.address, 0, 1, accounts[1], accounts[1], {"from": accounts[1], "value": pay_amount})
