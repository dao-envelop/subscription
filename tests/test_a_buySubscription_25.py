...
def test_buy_subscription(accounts, dai, weth, sub_reg, minter1, EnvelopAgentWithRegistry, TokenMock, wrapper, wnft721):
	...
	payOptions = [(niftsy.address, 1, 1000 )] #without Agent fee
	...
	niftsy.transfer(accounts[1], 1, {"from": accounts[0]})
	niftsy.approve(sub_reg.address, 1, {"from": accounts[1]})



	tx = agent.buySubscription(minter1.address, 0, 0, accounts[2], accounts[1], {"from": accounts[1]})
